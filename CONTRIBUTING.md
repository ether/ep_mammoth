# Contributing to ep_mammoth

Thanks for helping improve `ep_mammoth` — a plugin for [Etherpad](https://etherpad.org/). LLM/Agent contributions are explicitly welcome, provided they follow the rules below.
...
`ep_mammoth` is a plugin, so you develop it inside a checkout of [ether/etherpad](https://github.com/ether/etherpad).
...
# Install ep_mammoth from your local clone
pnpm run plugins i --path ../ep_mammoth

# Start the dev server (port 9001)
pnpm --filter ep_etherpad-lite run dev
```

Edits in `../ep_mammoth/` are picked up after a server restart.
...
* Etherpad version, ep_mammoth version, browser + OS
* Steps to reproduce
